import random
import math
import numpy as np
import matplotlib.pyplot as pp


# default function to initialize weights and biases
def rand():
    return random.uniform(-1, 1)


# basic activation function
def sigmoid(x):
    return 1 / (1 + math.exp(-x))


def _tanh(x: int):
    return 1 - sigmoid(-2 * x)


vectorized_tanh = np.vectorize(_tanh)


# default activation function
def tanh(v: np.ndarray):
    return vectorized_tanh(v)


vectorized_relu = np.vectorize(lambda x: max(0, x))


def relu(v: np.ndarray):
    return vectorized_relu(v)


def softmax(v):
    w = np.copy(v)
    for i in range(w.size):
        w[(i, 0)] = math.e ** w[(i, 0)]
    return w / sum(w)


# calculates gradient of a function at a specific value, used for descent
def gradient(func, x, delta=0.001):
    vectorized_func = np.vectorize(func)
    return (func(x + delta) - func(x)) / delta


class Net:
    def __init__(self, nodes: list, act=[]):
        self.layers = len(nodes) - 1
        self.nodes = nodes
        self.cons = []
        self.biases = []
        self.activate = act
        np.random.seed(101)

        if len(self.activate) == 0:
            self.activate = [tanh for i in range(self.layers + 1)]
            self.activate[0] = tanh
            self.activate[-1] = softmax

        for layer in range(self.layers):
            self.cons.append(np.random.uniform(-1, 1, (nodes[layer + 1], nodes[layer])))
            self.biases.append(np.random.uniform(-1, 1, (nodes[layer + 1], 1)))

    # change the activation function
    def set_activation(self, act):
        self.activate = act

    # returns a string containing formatted weights and biases arrays
    def __str__(self):
        return str(self.cons) + "\n\n" + str(self.biases)

    # feeds forward an input np array and returns the calculated output
    def get_out(self, inp: np.ndarray):
        for layer in range(self.layers):
            # inp = np.vectorize(self.activate[layer])(inp)
            inp = self.activate[layer](inp)
            inp = np.matmul(self.cons[layer], inp) + self.biases[layer]
        # return np.vectorize(self.activate[-1])(inp)
        return self.activate[-1](inp)

    # trains the net on a given input array, desired output array, and learning rate (default 0.1)
    def train(self, inp: np.ndarray, exp: np.ndarray, lr=0.1):
        inputs = [inp]
        for layer in range(self.layers):
            # inp = np.vectorize(self.activate[layer])(inp)
            inp = self.activate[layer](inp)
            inp = np.matmul(self.cons[layer], inp) + self.biases[layer]
            inputs.append(inp)

        # inputs has all node values from inp to out (not activated)

        # has been activated
        # out = np.vectorize(self.activate[-1])(inp)
        out = self.activate[-1](inp)

        # errors will have all errors from out to first hidden in that order (activated)
        errors = [exp - out]

        for layer in range(self.layers - 1):
            errors.append(np.matmul(np.transpose(self.cons[-(layer + 1)]), errors[layer]))

        for layer in range(self.layers):
            # adjust connection weights
            self.cons[-(layer + 1)] += np.matmul(
                lr * errors[layer] * gradient(self.activate[-(layer + 1)], inputs[-(layer + 1)]),
                np.transpose(inputs[-(layer + 2)]))
            # adjust bias weights
            self.biases[-(layer + 1)] += lr * errors[layer] * gradient(
                self.activate[-(layer + 1)], inputs[-(layer + 1)])

# import random
# import math
# import numpy as np
# import matplotlib.pyplot as pp
#
#
# # default function to initialize weights and biases
# def rand():
#     return random.uniform(-1, 1)
#
#
# # basic activation function
# def sigmoid(x: int):
#     try:
#         return 1 / (1 + math.exp(-x))
#     except OverflowError:
#         return 0
#
#
# # default activation function
# def tanh(v: np.ndarray):
#     w = np.copy(v)
#     for i in range(w.size):
#         w[(i, 0)] = 1 - sigmoid(-2 * w[(i, 0)])
#     return w
#
#
# # relu
# def relu(x: int):
#     return max(0, x)
# relu = np.vectorize(relu)
#
#
# def softmax(v: np.ndarray):
#     w = np.copy(v)
#     for i in range(w.size):
#         w[(i, 0)] = math.e ** w[(i, 0)]
#     return w / sum(w)
#
#
# # calculates gradient of a function at a specific value, used for descent
# def gradient(func, x, delta=0.001):
#     return (func(x + delta) - func(x)) / delta
#
#
# class Net:
#     def __init__(self, layers, nodes, act=[], r=rand):
#         self.layers = layers - 1
#         self.nodes = nodes
#         self.cons = []
#         self.biases = []
#         self.activate = act
#
#         if self.activate == []:
#             self.activate = [tanh for i in range(self.layers + 1)]
#             self.activate[0] = tanh
#             self.activate[-1] = softmax
#
#         for layer in range(self.layers):
#             self.cons.append(np.random.uniform(-1, 1, (nodes[layer + 1], nodes[layer])))
#             self.biases.append(np.random.uniform(-1, 1, (nodes[layer + 1], 1)))
#
#     # change the activation function
#     def set_activation(self, act):
#         self.activate = act
#
#     # returns a string containing formatted weights and biases arrays
#     def __str__(self):
#         return str(self.cons) + "\n\n" + str(self.biases)
#
#     # feeds forward an input np array and returns the calculated output
#     def get_out(self, inp):
#         for layer in range(self.layers):
#             inp = self.activate[layer](inp)
#             inp = np.matmul(self.cons[layer], inp) + self.biases[layer]
#         return self.activate[-1](inp)
#
#     # trains the net on a given input array, desired output array, and learning rate (default 0.1)
#     def train(self, inp, exp, lr=0.1):
#         inputs = [inp]
#         for layer in range(self.layers):
#             inp = self.activate[layer](inp)
#             inp = np.matmul(self.cons[layer], inp) + self.biases[layer]
#             inputs.append(inp)
#
#         # inputs has all node values from inp to out (not activated)
#
#         # has been activated
#         out = self.activate[-1](inp)
#
#         # errors will have all errors from out to first hidden in that order (activated)
#         errors = [exp - out]
#
#         for layer in range(self.layers - 1):
#             errors.append(np.matmul(np.transpose(self.cons[-(layer + 1)]), errors[layer]))
#
#         for layer in range(self.layers):
#             # adjust connection weights
#             self.cons[-(layer + 1)] += np.matmul(
#                 lr * errors[layer] * gradient(self.activate[-(layer + 1)], inputs[-(layer + 1)]),
#                 np.transpose(inputs[-(layer + 2)]))
#             # adjust bias weights
#             self.biases[-(layer + 1)] += lr * errors[layer] * gradient(
#                 self.activate[-(layer + 1)], inputs[-(layer + 1)])
